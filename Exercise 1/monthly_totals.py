...
def pivot_months_pandas(data):
    ...
    """
    # ...
    # reference from https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.apply.html
    data_date = data.date
    month = data_date.apply(date_to_month)
    data['month'] = month
    # print(month)

    grouped_data = data.groupby(['name', 'month'])
    aggregate_totals = grouped_data.aggregate('sum').reset_index()

    monthly = aggregate_totals.pivot(index='name', columns='month')['precipitation']
    # print(monthly)

    aggregate_counts = grouped_data.aggregate('count').reset_index()

    counts = aggregate_counts.pivot(index='name', columns='month')['precipitation']
    # print(counts)
