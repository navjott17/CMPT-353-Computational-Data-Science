...
def main(infile):
    ...
    # reference from https://datatofish.com/dataframe-to-numpy-array/
    df_x = pd.DataFrame(data, columns=['R', 'G', 'B'])
    df_x = df_x/255
    array_x = df_x.to_numpy()
    array_y = data['Label'].to_numpy()
    X = array_x # array with shape (n, 3). Divide by 255 so components are all 0-1.
    y = array_y # array with shape (n,) of colour words.
